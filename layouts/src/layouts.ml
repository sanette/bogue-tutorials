...
(**
   ...
   Let's see what this gives. We may create a Bogue app with this sole "house",
   as follows.
...
(**
   ...
   [house]. Then we have two {e branches}, corresponding to the groups contained
   in this window: [flat] and [tower]. The [flat] branch splits into three new
   ...
   contain any other group: in Bogue's terminology, they are called {e
   resident}; in the tree language, they are {e leaves}. Leaves always contain
   {b widgets}. (Here, the "text" layout contains a [label] widget, and the
   "image" layout contains a [box] widget.)
   ...
   In Bogue, layouts are used to position the widgets in the window. The set of
   all layouts form a tree.  A layout can either contain a widget, in which case
   it is a leaf, or a list of layouts, in which case it is a node. We often call
   such a node a {e house}, and the layouts in the list are the {e rooms}.
   ...
   layout we construct. The final Bogue app is constructed from this trunk,
   using
   ...
   {{:http://sanette.github.io/bogue/Bogue.Main.html#VALof_layouts}[Bogue.of_layouts]}.
   +SIDE:end

   {1 Dynamical layouts}

   The tree structure we've just described is perfect for {e static}
   layouts. When designing our app, we can sketch the GUI on a piece of paper,
   find out the corresponding tree, and then implement it in Bogue, starting
   from the leaves, and grouping them step by step, up to the trunk.

   However, very often, GUI are nicer when they have {e dynamic} (or
   "responsive") components. For instance, what happens if Layout [r3], in
   response to some user interaction, wants to modify Layout [flat], to which it
   belongs?  This is the subject of the {{!page-modif_parent}Self-modifying
   layouts} tutorial.
